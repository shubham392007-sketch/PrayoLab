...
export function ComputedLab({ title, subtitle, fields, compute }: ComputedLabProps) {
  ...
  return (
    <LabShell title={title} subtitle={subtitle}>
      <div className="grid lg:grid-cols-[340px_1fr] gap-6">
        <div className="pl-card pl-soft-shadow p-5 space-y-4">
          ...
          <Button onClick={() => { try { setOut(compute(vals)); } catch (e: any) { setOut({ steps: [{ title: "Error", note: e?.message }] }); } }} className="w-full rounded-full bg-primary text-primary-foreground hover:bg-primary/90">Solve</Button>
        </div>
